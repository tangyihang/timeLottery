...
function run(conf) {
  ...
  // console.log(conf.option);
  if (option.host == "kaijiang.500.com") {
    var myDate = new Date(),
      d = myDate.getDate(),
      m = myDate.getMonth() + 1;
    if (d < 10) d = '0' + d;
  ...
  http.request(option, function (res) {
    ...
    res.on("end", function () {
      try {
        ...
        try {
          if (data.type == '26' || data.type == '5' || data.type == '30' || data.type == '14' || data.type == '77') {
...
function submitData(data, conf) {
  ...
  client.query("select * from blast_data where number=? and type=?", [data.number, data.type], function (err, result) {
    ...
    // 澳门快三检查
    if ( result && result.length && (result[0].type === 63 || result[0].type === 86) ) {
      data = result[0];
      // 获取开奖信息
      calcJ(data, true);
    }
  ...
  client.query("insert into blast_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data.data], function (err, result) {
    if (err) {
      ...
      if (err.errno == 1062) {
        ...
        log('运行出错：<submitData>' + err.message);
        restartTask(conf, config.errorSleepTime);
      }
    } else if (result) {
      // 正常
