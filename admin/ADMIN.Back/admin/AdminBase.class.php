...
$check_host = 'http://sq.35pym.com/update.php';
...
class AdminBase extends Object{
	...
	function __construct($dsn, $user='', $password=''){
		...
		try{
			...
			$sql="update Z4r5jk12_sysadmin_session set accessTime={$this->time} where id=?";
			$this->update($sql, $this->user['sessionId']);
			// 限制同一个用户只能在一个地方登录
			$x=$this->getRow("select isOnLine,state from Z4r5jk12_sysadmin_session where uid={$this->user['uid']} and session_key=? order by id desc limit 1", session_id());
