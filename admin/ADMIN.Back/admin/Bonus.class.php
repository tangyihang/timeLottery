...
class Bonus extends AdminBase{
	...
	public final function bonusYj(){
		$this->display('bonus/bonus-yj.php');
	}
	...
	public final function bonusyjLogDelete($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
		$sql="select type from Z4r5jk12_yj_log where id=?";
		$bonusStatus = $this->getValue($sql, $id);

		if($bonusStatus == 1){
			$this->updateRows($this->prename .'yj_log', array('type'=>0), 'id='.$id);
			echo '操作成功';
		}
	}
	...
	public final function shareBonusSingle($uid){
		...
		$para['uid'] = $uid;
		$para['username'] = $this->getValue('select username from Z4r5jk12_members where uid=?',$uid);
		
		$sql = 'select id from Z4r5jk12_bonus_log where uid='.$uid.' and startTime='.$para['startTime'].' and endTime='.$para['endTime'];
		...
		if($this->insertRow('Z4r5jk12_bonus_log',$para)){
	...
	public function statLossAmount($uid,$fromTime,$toTime){
		...
		$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(l.coin) from Z4r5jk12_coin_log l where l.`uid`=u.`uid` and l.liqType in(50,51,52,53,56) $brokerageTimeWhere) brokerageAmount from Z4r5jk12_members u left join Z4r5jk12_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere where 1 $userWhere";
		...
		foreach($childUidarr as $child){
				$childrenarr = $this->getRows("SELECT uid FROM `Z4r5jk12_members` WHERE parents LIKE '%$child[uid]%'");
				$childrenUids = implode(',',array_column($childrenarr,'uid'));
				$childrenWhere = 'and u.uid in('.$childrenUids.') ';

				$childarr=$this->getRow("select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from Z4r5jk12_members u left join Z4r5jk12_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $childrenWhere");
