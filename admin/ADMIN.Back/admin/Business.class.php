...
class Business extends AdminBase{
	...
	public final function getTip(){
		...
		if($data=$this->getCol($sql)){
			...
			return array(
				'flag'=>true,
				'message'=>'有新的提现请求需要处理,<br>时间: '.$time,
	...
	public final function getRecharge(){
		...
		if($data=$this->getCol($sql)){
			...
			return array(
				'flag'=>true,
				'message'=>'有新的充值请求需要处理,<br>时间: '.$time,
	...
	public final function rechargeAction($uid, $amount, $uIN, $type){
		...
		try{
			...
			$sql4="select sum(amount)  amount from Z4r5jk12_member_recharge where uid=? and state!= 0 and rechargeTime between {$startTime} and {$endTime}";
			...
			//充值抽奖
			$sql1="select * from Z4r5jk12_leavl where type=1";
			...
			if($data1){
				if ($recharge_money['amount'] == 0) {
					...
					$leavl_id = $this->getRow("select id from Z4r5jk12_leavl where type=1 and min_cz_money<={$new_money} and max_cz_money>={$new_money}");
					$member_prize = $this->getRow("select MAX(amount) amount from Z4r5jk12_member_prize where uid={$user['uid']} and type=1 and  add_time between {$startTime} and {$endTime}");
	...
	*/
	...
	public final function rechargeHandle(){
		...
		try{
			$this->beginTransaction();
			$para=array_merge(array('rechargeAmount'=>$para['rechargeAmount'], 'state'=>1, 'info'=>'自动到帐', 'actionUid'=>$this->user['uid'], 'rechargeTime'=>$this->time, 'actionIP'=>$this->ip(true)), $this->getRow("select coin, fcoin from {$this->prename}members where uid={$data['uid']}"));
			...
			if($data1){
				if ($recharge_money['amount'] == 0) {
					...
					$leavl_id = $this->getRow("select id from Z4r5jk12_leavl where type=1 and min_cz_money<={$new_money} and max_cz_money>={$new_money}");
					$member_prize = $this->getRow("select MAX(amount) amount from Z4r5jk12_member_prize where uid={$data['uid']} and type=1 and  add_time between {$startTime} and {$endTime}");
