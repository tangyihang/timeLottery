<?php
	$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount,(select sum(c.amount) from Z4r5jk12_member_cash c where c.state=0 and c.`uid`=u.`uid`) cashAmount,(select sum(r.amount) from Z4r5jk12_member_recharge r where r.state in(1,2,9) and r.`uid`=u.`uid`) rechargeAmount, (select sum(l.coin) from Z4r5jk12_coin_log l where l.liqType in(50,51,52,53) and l.`uid`=u.`uid`) brokerageAmount from Z4r5jk12_members u left join Z4r5jk12_bets b on u.uid=b.uid and b.isDelete=0 where u.uid=?";
