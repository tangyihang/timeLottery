<?php
	$sql="select u.*, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from Z4r5jk12_member_cash c where c.state=0 and c.`uid`=u.`uid`) cashAmount,(select sum(r.amount) from Z4r5jk12_member_recharge r where r.state in(1,2,9) and r.`uid`=u.`uid`) rechargeAmount from Z4r5jk12_members u left join Z4r5jk12_bets b on u.uid=b.uid and b.isDelete=0 where u.admin=0";
	...
	$sql="select sum(coin) from Z4r5jk12_coin_log where uid=? and liqType between 2 and 3";
...
?>
    ...
    <tr> 
        ...
        <td><?=$var['coin']?>/<?=$var['fcoin']?><span class='spn10'></td> 
        <td><?=$var['fanDian']?>%</td> 
        <td><?=$this->iff($login['isOnLine'] && ceil(strtotime(date('Y-m-d H:i:s', time()))-strtotime(date('Y-m-d H:i:s',$login['accessTime'])))<300, '<font color="#FF0000">在线</font>', '离线')?></td>
