...
<input type="hidden" value="<?=$this->user['username']?>" />
    <header>
    	<h3 class="tabs_involved">中奖记录
            <div class="submit_link wz">
                ...
                时间：从 <input type="date" style="width:120px;" name="fromTime"/> 到 <input type="date" style="width:120px;" name="toTime"/>&nbsp;&nbsp;
	...
	<tbody> 
		...
		<tr> 
			...
			<td>
            	<?php if($state!="完成"&&$var['enable']){?><a href="/index.php/dzp/pointState/<?=$var['id']?>/<?=$var['state']?>" target="ajax" call="pointHandle" dataType="json"><?=$statenext?></a> | <?php }?>
                
				<?php if($statenext!="【完成】"&&$state!="完成"){
					if($var['enable']){?>
                <a href="/index.php/dzp/pointEnable/<?=$var['id']?>/<?=$var['enable']?>" target="ajax" call="pointHandle" dataType="json">取消</a> |
				<?php }else{?>
                <a href="/index.php/dzp/pointEnable/<?=$var['id']?>/<?=$var['enable']?>" target="ajax" call="pointHandle" dataType="json">重启</a> |
                <?php }
				}?>
