...
function getIPLoc_sina($queryIP)
{
    ...
    if ($reg === FALSE) return "";
    $location = $reg->data;
    $loc = $location->country . $location->area . $location->region . $location->city;
    return $loc;
}
...
<article class="module width_full">
    ...
    <header>
        <h3 class="tabs_involved">登录日志
            <form action="/index.php/member/loginLog" target="ajax" dataType="html" call="defaultSearch"
                ...
                时间：从 <input type="date" class="alt_btn" name="fromTime"/> 到 <input type="date" class="alt_btn"
                                                                                   name="toTime"/>&nbsp;&nbsp;
