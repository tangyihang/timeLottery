...
<form action="/index.php/member/userUpdateed" target="ajax" method="post" call="userDataSubmitCode" onajax="userDataBeforeSubmitCode" dataType="html">
	...
	<table cellpadding="2" cellspacing="2" class="popupModal">
		<tr>
			...
			<td><input type="text" name="parents" value="<?=implode('> ',$this->getCol("select username from {$this->prename}members where uid in ({$userData['parents']})"))?>"/>&nbsp;<span class="spn9">置空为不修改</span></td>
		<tr>
		...
		<tr>
			<td class="title">推广地址：</td>
			<td><input type="text" name="spread" value="<?=$userData['spread']?>" />&nbsp;</td>
		</tr>
		<tr>
			<td class="title">wap推广地址：</td>
			<td><input type="text" name="wapspread" value="<?=$userData['wapspread']?>" />&nbsp;</td>
		</tr>
		<?php 
