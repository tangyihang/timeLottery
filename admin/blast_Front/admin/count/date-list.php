<?php
	...
	$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from blast_member_cash c where c.state=0 $cashTimeWhere and c.`uid`=u.`uid`) cashAmount,(select sum(r.amount) from blast_member_recharge r where r.state in(1,2,9) $rechargeTimeWhere and r.`uid`=u.`uid`) rechargeAmount, (select sum(l.coin) from blast_coin_log l where l.liqType in(50,51,52,53,56) $brokerageTimeWhere and l.`uid`=u.`uid`) brokerageAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere where 1 $userWhere";
	...
	$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from blast_members u left join blast_bets b on b.isDelete=0  $betTimeWhere $userWhere3 and u.uid=b.uid";
...
<input type="hidden" value="<?=$this->user['username']?>" />
	...
	<?php
		...
		if($var['username']!='没有下级了'){
			...
			$pId=$var['uid'];
			$var['teamwin']=$this->getValue("select sum(l.coin) fandianAll from {$this->prename}coin_log l, blast_members u where l.liqType in(2,3) $fanDiaTimeWhere and concat(',', u.parents, ',') like '%,$pId,%' and l.uid=u.uid") + $this->getValue("select sum(b.bonus-b.mode * b.beiShu * b.actionNum) betZjAmount from blast_members u ,blast_bets b where b.isDelete=0 $betTimeWhere and concat(',', u.parents, ',') like '%,$pId,%' and u.uid=b.uid");
		}
