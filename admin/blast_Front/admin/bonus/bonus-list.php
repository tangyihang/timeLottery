<?php
	...
	$sql="select uid,username,fanDian from blast_members WHERE (fanDian = 13.0 OR fanDian = 12.9)";
...
<article class="module width_full">
	...
	<tbody>
		<?php if($data) foreach($data as $var){ 
			...
			$bonusId = $this->getValue('select id from blast_bonus_log where uid='.$var['uid'].' and startTime='.$fromTime.' and endTime = '.$toTime);
			if($bonusId) continue;
