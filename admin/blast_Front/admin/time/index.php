...
<article class="module width_full">
	<header>
        ...
        <a button="确定增加:dataAddCode|取消:defaultCloseModal" modal="true" title="增加期数" style="padding-left:50px" width="500" target="modal" href="/index.php/time/add/<?=$this->type?>" class="alt_btn" >增加期数</a>
        <?php }?>
        </h3>
	</header>
...
-->                <td><input type="text" name="actionTime"   value="<?=$var['actionTime']?>"/></td>
				<td>
                 <?php if($this->type==34){ //六合彩?>
                 <a href="/index.php/time/updated/<?=$var['type']?>/<?=$var['id']?>" target="ajax" method="POST" onajax="sysBeforeUpdateTime" call="sysUpdateTime">保存修改</a>
				 <a href="/index.php/time/deleted/<?=$var['id']?>" target="ajax" onajax="beforeDelData" call="afterData" >清除</a>	
                 
                <?php }else{?>
                <a href="/index.php/time/updateTime/<?=$var['type']?>/<?=$var['id']?>" target="ajax" method="POST" onajax="sysBeforeUpdateTime" call="sysUpdateTime">保存修改</a>
                <?php }?>
                </td>
			</tr>
			<?php } ?>
