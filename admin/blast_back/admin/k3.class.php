<?php
class k3 extends AdminBase {
    public $pageSize = 15;

    // 玩法设置
    public final function rte($playid) {
        $playid = intval($playid);
        $this->display('k3/rte-list.php', 0, $playid);
    }

    public final function switchLHCRteStatus($id) {
        $id  = intval($id);
        $sql = "update {$this->prename}k3 set enable=not enable where id=?";
    ...
    public final function switchLHCRteMStatus($id) {
        $id  = intval($id);
        $sql = "update {$this->prename}k3 set android=not android where id=?";
        if ($this->update($sql, $id)) {
            $playName = $this->getValue("select name from {$this->prename}k3 where id=?", $id);
    ...
    public final function updateLHCRte($id) {
        $id = intval($id);
        if ($this->updateRows($this->prename . 'k3', $_POST, 'id=' . $id)) {
            $playName = $this->getValue("select name from {$this->prename}k3 where id=?", $id);
