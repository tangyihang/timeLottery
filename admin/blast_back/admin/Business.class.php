...
//, $actionFlag,
     ...
     * 二维码管理
     */
    public final function codeedit($id) {
        $this->display('business/codeedit.php', 0, $id);
    }
    ...
    public final function uploadImg($id) {
        $this->display('business/index.html', 0, $id);
    }
    ...
    //添加二维码数据
    public final function addQrcodeAction($name, $title, $account, $type) {
        $data = array(
            'name'    => $name,
            'title'   => $title,
            'account' => $account,
    ...
    public final function rechargeAction($uid, $amount, $uIN, $type) {
        ...
        try {
            ...
            if (!$recharge_money['amount']) {
                $recharge_money['amount'] = 0;
            }
    ...
    public final function betinfoUpdateed() {
        ...
        if (($usercoin + $betcoin) < $coin) {
            throw new Exception('该用户的余额不足以投注这单,请减小数值');
        }

        $endcoin = $betcoin + $usercoin - $coin;
        $coins   = -$coin;
