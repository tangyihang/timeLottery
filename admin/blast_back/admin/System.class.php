...
class System extends AdminBase
{
    ...
    public final function updateSettings()
    {
        ...
        if (!is_numeric($para['sign_num'])) throw new Exception('请正确设置签到奖励!');
...
//完	
    ...
    //优惠活动
    public final function actlist()
    {
        $this->display('system/act-list.php');
    }



    public final function updateActive($id)
    {
        $this->display('system/active-update.php', 0, $id);
    }

    public final function deleteActive($id)
    {
        if (!$id = intval($id)) throw new Exception('参数出错');
        $sql = "delete from {$this->prename}active where id=?";
        if ($this->delete($sql, $id)) {
            echo '已经删除';
        } else {
            throw new Exception('未知出错');
    ...
    public final function doUpdateActive($id)
    {

        if (!$_POST['title']) throw new Exception('活动标题不能为空');
        if (!$_POST['obj']) throw new Exception('活动对象不能为空');
        if (!$_POST['content']) throw new Exception('活动内容不能为空');
        if (!$_POST['text']) throw new Exception('活动简介不能为空');

        if ($this->updateRows($this->prename . 'active', $_POST, 'id=' . $id)) {
            return '修改成功';
        } else {
            throw new Exception('未知出错');
        }
    ...
    public final function uploadify()
    {
        ...
        if(is_uploaded_file($_FILES['photo']['tmp_name'])){
            ...
            $imgdir = $dir.$filename;
            $id = $_POST['id'];
            $sql = "update {$this->prename}active set img='{$imgdir}' where id=?";
            if ($this->update($sql, $id))
                echo json_encode(array('code' => 200, 'dir' => $imgdir, 'msg' => '上传成功'));
            else
               echo json_encode(array('code' => 404, 'msg' => '上传失败'));
        }else{
            echo json_encode(array('code' => 404, 'msg' => '上传失败'));
