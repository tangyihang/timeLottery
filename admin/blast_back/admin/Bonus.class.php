...
class Bonus extends AdminBase{
	...
	public final function bonusyjLogDelete($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
		$sql="select type from blast_yj_log where id=?";
		$bonusStatus = $this->getValue($sql, $id);
	...
	public final function shareBonusSingle($uid){
		...
		$para['uid'] = $uid;
		$para['username'] = $this->getValue('select username from blast_members where uid=?',$uid);
		
		$sql = 'select id from blast_bonus_log where uid='.$uid.' and startTime='.$para['startTime'].' and endTime='.$para['endTime'];
		...
		if($this->insertRow('blast_bonus_log',$para)){
	...
	public function statLossAmount($uid,$fromTime,$toTime){
		...
		$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(l.coin) from blast_coin_log l where l.`uid`=u.`uid` and l.liqType in(50,51,52,53,56) $brokerageTimeWhere) brokerageAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere where 1 $userWhere";
		...
		foreach($childUidarr as $child){
				$childrenarr = $this->getRows("SELECT uid FROM `blast_members` WHERE parents LIKE '%$child[uid]%'");
				$childrenUids = implode(',',array_column($childrenarr,'uid'));
				$childrenWhere = 'and u.uid in('.$childrenUids.') ';

				$childarr=$this->getRow("select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $childrenWhere");
