...
class Commission extends AdminBase{
	...
	public final function conComSingle($uid){
		...
		//exit;
		$sql="select u.username, u.coin, u.uid, u.type, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 and b.actionTime between $fromTime and $toTime where 1 and u.uid={$uid} group by u.uid";
	...
	public final function lossComSingle($uid){
		...
		//echo floatval($this->settings['conCommissionBase1']);
		//exit;
			$sql="select u.username, u.coin, u.uid, u.type, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(coin) from blast_coin_log l where l.`uid`=u.`uid` and liqType in(2,3) and l.actionTime between $fromTime and $toTime) fanDianAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 and b.actionTime between $fromTime and $toTime where 1 and u.uid={$uid} group by u.uid";
			$res = $this->getRows($sql);
			$userloss = $res[0];
