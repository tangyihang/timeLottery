...
<div class="scorllmain-content scorll-order top_bar bottom_bar" style="display:<?php if ($imgurl != '') {
    echo 'none';
} else {
    echo 'block';
} ?>"><?= $error ?></div>
<!-- 微信支付第二步 - 扫码 -->
<div id="wechat_step2_scan" class="scorllmain-content scorll-order top_bar bottom_bar">
    <div class="wechat-order">
        <div class="we-or-tit">
            <h3>订单提交成功，请扫描以下二维码付款！</h3>
            <p>
                编码：<span class="red" name="orderId" id="orderId_qr"><?= $args[0]['rechargeId'] ?></span><span
                        class="copy_outer"><a id="wx_Pay" data-cp="orderId_2_s" name="cp_btn"
                                              class="we-blue">复制</a></span>　|　应付金额：<span class="red"
                                                                                         id="qrmoney"><?= $args[0]['amount'] ?></span>元
            </p>
        </div>
        <div id="code"><img style="width: 350px;" src="http://admin.222345678.com<?= $data['imgaddr'] ?>"></div>
        <div>
        </div>
    </div>
    ...
    <div class="pay-info-tit">
        ...
        <div class="tips f12">
            <p>※ 1.截图保存二维码</p>
            <p>※ 2.手机登录<?php if ($args[0]['pay_type'] == 10) {
                    echo '微信';
                } else if ($args[0]['pay_type'] == 20) {
                    echo "支付宝";
                } else if ($args[0]['pay_type'] == 50) {
                    echo "QQ";
                } ?>.</p>
            <p>※ 3.打开<?php if ($args[0]['pay_type'] == 10) {
                    echo "微信";
                } else if ($args[0]['pay_type'] == 20) {
                    echo "支付宝";
                } else if ($args[0]['pay_type'] == 50) {
