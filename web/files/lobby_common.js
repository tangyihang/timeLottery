...
window._lottery_menu = {
    ...
    leftMenu : function (){
       /*  $.ajax({
        ...
        ///** 【热门彩种】彩票menu ，张开和收起  **/
        window.MainTitTime = 0;
        window.MainTopTime = 0;
        window.parntTime = 0;
        $('#hot_lottery').delegate('.main-tit','click',function(evt){
        
        	var times = new Date().getTime();
        	if((times - MainTitTime)<100){
        		return;
        	}else{
        		MainTitTime = times;
        	}
        ...
        ///** 【高频,低频】彩票menu ，张开和收起  **/
        $('#live-main-list .main-top').on("click",function(evt){
			
			var times = new Date().getTime();
        	if((times - MainTopTime)<100){
        		return;
        	}else{
        		MainTopTime = times;
        	}
        ...
        ///** 【高频二级菜单】彩票menu ，张开和收起  **/
        $('#live-main-list  ul.gao-ul  .icon-attr-down').parent().bind("click",function(evt){  
        	var times = new Date().getTime();
        	if((times - parntTime)<100){
        		return;
        	}else{
        		parntTime = times;
        	}
...
//打开注册登录弹窗
function opRegDIV(){
    if( $("#pup_REG_div_t").length == 0 )
    {
        $("body").append("<div onclick=\"gb(this)\" id=\"pup_REG_div_t\"><div><iframe src=\"/index.php/user/login_t\" name=\"_iframe_reg\" scrolling=\"no\" marginwidth=\"0px\" marginwidth=\"0px\" frameborder=\"0\"  samless ></iframe></div></div>");
        $("#pup_REG_div_t").css({width:"100%",height:"100%",position:"fixed",zIndex:"15",backgroundColor:"rgba(30,30,30,0.5)",left:"0",top:"0"});
       $("#pup_REG_div_t>div").css({width:"650px",marginLeft:"-325px",height:"520px",marginTop:"-260px",position:"absolute",left:"50%",top:"49%"});
        $("#pup_REG_div_t iframe").css({width:"100%",height:"100%",margin:"0",display:"block"});
    }
    else
    {
        $("#pup_REG_div_t iframe").attr("src","/index.php/user/login_t");
    }
    $("#pup_REG_div_t").show();
...
function opFreePalyDIV(){
    ...
    {
        $("body").append("<div id=\"__pup_REG_div\"><div><iframe src=\"/index.php/user/registertest\" name=\"_iframe_reg\" scrolling=\"no\" marginwidth=\"0px\" marginwidth=\"0px\" frameborder=\"0\"  samless ></iframe></div></div>");
    ...
    {
        $("#__pup_REG_div iframe").attr("src","/index.php/user/registertest");
    }
    $("#__pup_REG_div").hide();
    window.location.reload();
}
