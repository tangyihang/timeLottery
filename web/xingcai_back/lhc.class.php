...
class lhc extends WebLoginBase
{
    public final function hklhc_zd()
    {
        $type = $_REQUEST['type'];
        ...
        $ch = curl_init('http://www.9800.com.tw/');
        ...
        $conta=preg_replace(array('/[\r\n]| class=".*?"|	/','/<tr><td>(\d{6})<\/td><td>([\d\,]{59})<\/td><td>(\d{2})<\/td><td>([\d\-\: ]{16})<\/td><\/tr>/','/<tr>.*?<\/tr>/'),array('','<row expect="'.$lastNo['actionNo'].'" opencode="$2" opentime="'.$lastNo['actionTime'].'" />',''),$conta);
    ...
    public final function hklhc_sd()
    {
        $type = $_REQUEST['type'];
        $lastNo = $this->getGameLastNo($type);
    }
