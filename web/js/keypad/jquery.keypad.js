...
function Keypad() {
	...
	this.regional[''] = { // Default regional settings
		...
		buttonStatus: '�򿪼���', // Status text for trigger button
		closeText: '�ر�', // Display text for close link
		closeStatus: '�رռ���', // Status text for close link
...
$.extend(Keypad.prototype, {
	...
	_disableKeypad: function(target) {
		...
		target.disabled = true;
			...
			control.prepend('<div class="' + this._disableClass + '" style="width: ' +
				...
				'px; left: 0px; top: ' + (offset.top - relOffset.top) + 'px;"></div>');
			var inst = $.data(target, PROP_NAME);
	...
	_showKeypad: function(field) {
        if(document.activeElement.id!="secpass"){
		...
		inst._mainDiv.css({position: (isFixed ? 'fixed' : 'absolute'), display: 'none',
			left: offset.left-180 + 'px', top: offset.top + 'px'});
		var showAnim = $.keypad._get(inst, 'showAnim') || 'fadeIn';
