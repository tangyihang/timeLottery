...
*/
	...
	$merchant_private_key=$args[0]['merchant_private_key'];
	$merchant_code = $args[0]['merchant_code'];//   "1000772890";//商户号，1111110166是测试商户号，线上发布时要更换商家自己的商户号！
	...
	$notify_url =$args[0]['notify_url'];// "http://15l0549c66.iask.in:45191/testnewb2c/offline_notify.php";		
	
	$order_no = $args[0]['order_no'];//date( 'YmdHis' );	

	$order_time =date( 'Y-m-d H:i:s' );	 //

	$order_amount =$args[0]['order_amount'];// "0.1";	

	$product_name =$args[0]['product_name'];//"testpay";	

	//以下参数为可选参数，如有需要，可参考文档设定参数值
	
	$return_url ="";	
	
	$pay_type = $args[0]['banktype'];
...
/////////////////////////////   获取sign值（RSA-S加密）  /////////////////////////////////
	...
	$str= openssl_get_privatekey($merchant_private_key);
/*	$str= chunk_split($merchant_private_key, 64, "\n");
	...
	$sign_info='';
	openssl_sign($signStr,$sign_info,$str,OPENSSL_ALGO_MD5);
...
<html>
	...
	<body >
		<form id="form1" name="dinpayForm" method="post" action="https://pay.ddbill.com/gateway?input_charset=UTF-8" >
