<?php
	...
	$this->bet = $_REQUEST['bet'];
	$this->dc = $_REQUEST['dc'];
	$this->my = $_REQUEST['my'];
	$this->gdid = $_REQUEST['gdid'];
	...
	if($this->type==20){//北京pk10
		$logcls = "icon_bj_pk10";
        $zst_url = '/zst/pk10.php?typeid='.$this->type;
	}	
	...
	if($this->type==77 || $this->type==34){//六合彩
		$logcls = "icon_xglhc";
        $zst_url = '/zst/lhc.php?typeid='.$this->type;
	}		
	if($this->type==80){//幸运28
		$logcls = "icon_bjkl8";
	}		
	if($this->type==83){//北京28
		$logcls = "icon_bj28";
	}
    if($this->type==87){//上海时时乐
        $logcls = "icon_shssl";
    }
    if($this->type==85){//三分pk10
        $logcls = "icon_sfpk10";
        $zst_url = '/zst/pk10.php?typeid='.$this->type;
    }
?>
<div style="position: relative;" class="lottery_head" id="kaijiang" type="<?=$this->type?>" ctype="<?=$types[$this->type]['type']?>">
	<div class="header-tab">
		<div class="jyq active" target='.lottery_num' ><span>近一期</span></div>
		<!--<div class="usercenter" target='.tools'><span>个人中心</span></div>-->
	
		<div class='jwq' target='.wq_data'><span>近五期</span></div>
	
	</div>
	<div id="wq_data" class="wq_data">
			<ul>
		...
		<div class="lottery_con">
			...
			<div class="lottery_timer left">
				<div class="lottery_issue left">
				    <h3 class='lottery-name _black'><?=$types[$this->type]['title']?></h3>
					<p style='color:#666;' class='_black'>第 <span id="current_issue" class='_red'><?=$actionNo['actionNo']?></span> 期<?php if($types[$this->type]['num'] > 0){?><p class='_black'>总共：<span class='_red'><?=$types[$this->type]['num']?></span> 期</p><?php }?></p>

					<!--<p class='_black'>投注剩余时间</p>-->
					<!--<span class="soundBtn soundClickEvent" onclick="voiceKJ();"></span>-->
					<a class="code-step" href="<?php echo $zst_url?$zst_url:'/zst?typeid='.$this->type;?>"  target="_blank">走势分析</a>
				</div>
 ...
 <?php if($types[$this->type]['type']==3) { //3D?>
	...
	<div class="tools left ">
		<div class="yu_e">
			...
			<a href="javascript:loginout()" style="color:#FFF">退出</a>
		</div>
		<div class="toolslist">
			<ul class="ctqc">
...
<?php }else if($types[$this->type]['type']==11) { //六合彩?>
				...
				<div class="louhao">
					<a class="wanfa"  href="<?php echo $zst_url?$zst_url:'/zst?typeid='.$this->type;?>" target="_blank">走势分析</a>
				</div>
			</div>
...
<script>
	var _type = '<?=$this->type?>';
	var _number = '<?=$lastNo['actionNo']?>';

	var fiveCb = function(data){
		var html = '';
		data = JSON.parse(data);
		data.results.forEach(function(item){
			var codes = item.data.split(",");
			html += '<li>';
				html += '<p class="qi_num">第<span>'+item['number']+'</span>期</p>';
					html += '<p class="qi_code">'
						codes.forEach(function(num){
							html +='<i>'+num+'</i>'
						})
				  html += "</p>";
			html += '</li>';
		});
		$("#wq_data ul").html(html);
	};
	$(".jwq").click(function(){
		getFiveData(_type,fiveCb,_number);
	})
	//getFiveData(_type,fiveCb);
