...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="grayTable">
	...
	<tbody>
	  <?php if($list['data']) foreach($list['data'] as $var){ ?>
		 <tr class="viewbox" tourl="/index.php/box/detail/<?=$var['mid']?>" dataid="<?=$var['mid']?>" >
			<td><INPUT  type="checkbox" value="<?=$var['mid']?>" name="chk_only" style="height: 19px;"></td>
	...
	<?php } ?>
		<tr>
    	  <td colspan="5" class="tl" style="text-align: left; text-indent: 20px;">
              <label><INPUT  id="chk_All" type="checkbox" value="All" name="chk_All" style="height: 19px;"></label>&nbsp&nbsp
